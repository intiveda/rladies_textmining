...
- Librerías: ```tm, tidytext, tidyverse, dplyr, datatable, ggplot, igraph, ggraph, worldcloud```
...
```{r subtools, message=TRUE, warning=FALSE}
...
TDM <- as.matrix(TDM)
vec.season <- c(rep(x = 1,13), rep(2, 22), rep(3,24), rep(4,22), rep(5,24), rep(6,25)) #episodios por temp
TDM.season <- t(apply(TDM, 1, function(x) tapply(x, vec.season, sum)))
colnames(TDM.season) <- paste("S", unique(vec.season))
```
...
## Anexos y otra információn útil
+ http://www.pieceofk.fr/?p=437
+ https://rstudio-pubs-static.s3.amazonaws.com/180610_d3764c43f1e54692b7e84d21ec94772a.html
