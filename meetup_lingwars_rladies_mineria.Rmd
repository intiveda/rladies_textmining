...
* Chequea que tienes las librerías instaladas. No te apures si worldcloud o knit, aparecen como no instaladas, es posible que no estén disponibles para tu versión R 
...
El segundo paso, una vez preparado nuestro corpus, será el de construir la matriz de términos documentos. Para simplificar el análisis cada temporada constituirá un documento. 
De esta forma obtenemos para cada término la frecuencia por temporada. 
...
tf_idf_df %>% 
  ...
  coord_flip()
```
```{r byseason, fig.height=8, fig.width=8,}
tf_idf_df %>% 
  group_by(season) %>% 
  top_n(8) %>% 
  ungroup %>%
...
```
```{r ,fig.height=12, echo=TRUE, fig.width=12}
